...
const COMMAND_LINE_ARG_OPTIONS = [
  ...
  },
  {
    ...
    defaultOption: true,
    description:
      '{bold {italic REQUIRED}} Path to zip file and an {italic OPTIONAL} extract to path',
  ...
  },
  {
    ...
    description:
      'S3 bucket subfolder path without leading or trailing slashes.\n{italic Default: rapid-dicom-viewer}',
  },
  {
    name: 'nodelete',
    type: Boolean,
    alias: 'n',
    description: 'Do not delete existing files in the S3 subfolder',
  },
  {
    name: 'verbose',
    type: Boolean,
  ...
  },
  {
    ...
    type: Boolean,
    alias: 'k',
    description: 'Keep extracted files instead of automatically deleting',
  },
];
...
const USAGE_CONFIG = [
  {
    ...
    content:
      'A nodejs utility script to streamline cloud hosted rapid-dicom-viewer deployments to an S3 environment',
  },
  {
...
const REGEX_LEADING_TRAILING_SEP = new RegExp(`(^${sep})|(${sep}$)`, 'g');
...
const EXTRACT_DIRECTORY =
  OPTIONS.paths?.[1] ?? path.dirname(ZIP_FILE_PATH ?? '.');
...
async function deployZip() {
  ...
  for (let file of files) {
    ...
    try {
      ...
      log(
        `Error encountered attempting to upload: ${command.input.Key}`,
        LogLevel.Error,
        e
      );
    }
  ...
  if (!KEEP_EXTRACTED) {
    try {
      ...
      log(
        `Deleting extracted files: ${deletePath} - complete`,
        LogLevel.Verbose
      );
    } catch (e) {
      log(
        `Error encountered while deleting extracted files: ${deletePath} - start`,
        LogLevel.Error,
        e
      );
    }
...
(async function main() {
  if (DELETE_PREVIOUS) {
    try {
      ...
      log(
        'Error occurred attempting to delete previous deployment',
        LogLevel.Error,
        e
      );
    }
