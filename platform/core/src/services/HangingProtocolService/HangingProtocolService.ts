...
export default class HangingProtocolService extends PubSubService {
  ...
  public setProperties({studies, displaySets, activeStudy}, protocolId){
  ...
  public runHangingProtocol(protocolId, rerunLoader = false) {
    if (protocolId && typeof protocolId === 'string') {
      const protocol = this.getProtocolById(protocolId);
      this._setProtocol(protocol, { rerunLoader });
    }
  }

  public run(
    { studies, displaySets, activeStudy, rerunLoader = false },
    protocolId
  ) {
    this.studies = [...(studies || this.studies)];
    this.displaySets = displaySets;
    ...
    this._setProtocol(matchedProtocol, { rerunLoader });
  }
  ...
  getViewportsRequireUpdate(viewportIndex, displaySetInstanceUID) {
    ...
    protocolViewports.forEach((viewport, index) => {
      ...
      if (viewportNeedsUpdate) {
        ...
        const { displaySetInstanceUIDs, displaySetOptions } =
          viewport.displaySets.reduce(
            (acc, displaySet) => {
              const { id } = displaySet;

              let { displaySetInstanceUID: displaySetInstanceUIDToUse } =
  ...
  _updateViewports(options = null as HangingProtocol.SetProtocolOptions): void {
    ...
    if (this.customImageLoadPerformed) {
      if (options.rerunLoader) {
